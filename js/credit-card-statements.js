...
buttonFilter.addEventListener("click", () => {
    ...
    const contentDates = Array.from(uniqueDates, date => `<span class="select-none" data-value=${date}>${moment(date).format("DD/MM/YYYY")}</span>`).join('');
    const contentCategories = Array.from(uniqueCategories, category => `<span class="select-none" data-value="${category}">${category}</span>`).join('');
    const contentDescriptions = Array.from(uniqueDescriptions, description => `<span class="select-none" data-value="${description}">${description}</span>`).join('');
    const contentCards = Array.from(uniqueCards, card => `<span class="select-none" data-value="${card}">${card}</span>`).join('');
    const contentCustomers = Array.from(uniqueCustomers, customer => `<span class="select-none" data-value="${customer}">${customer}</span>`).join('');
    const contentValues = Array.from(uniqueValues, value => `<span class="select-none" data-value="${value}">${(value).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>`).join('');
